  ...
  return (
    <div className="relative">
      ...
      {defaultMovieHero?.map((movie) => (
        <div
          key={movie.imdbID}
          className="w-1/2 sm:w-full sm:items-center mx-auto absolute inset-0 flex flex-col items-center justify-center text-white m-4"
        >
          <h1 className="text-4xl font-bold text-center sm:text-left">
            {movie.Title}
          </h1>
          <p className="text-xl text-center sm:text-left">{movie.Year}</p>
          <p className="text-center xl:w-1/3 sm:w-full px-4">{movie.Description}</p>
